...
import { MAJIKAH_SQL_TABLE_MAJIK_KEY } from "../sql-schema";
...
export class SQLiteKeystoreAdapter implements MajikKeyStorageAdapter {
  ...
  async save(key: MajikKeyJSON): Promise<void> {
    await this.db.run(
      `INSERT OR REPLACE INTO ${MAJIKAH_SQL_TABLE_MAJIK_KEY} 
  ...
  async getById(id: string): Promise<MajikKeyJSON | null> {
    const row = await this.db.get<{ json: string }>(
      `SELECT json FROM ${MAJIKAH_SQL_TABLE_MAJIK_KEY} WHERE id = ?`,
      [id],
    );
  ...
  async list(): Promise<MajikKeyJSON[]> {
    const rows = await this.db.all<{ json: string }>(
      `SELECT json FROM ${MAJIKAH_SQL_TABLE_MAJIK_KEY}`,
    );
  ...
  async remove(id: string): Promise<boolean> {
    ...
    await this.db.run(
      `DELETE FROM ${MAJIKAH_SQL_TABLE_MAJIK_KEY} WHERE id = ?`,
      [id],
    );
  ...
  async clear(): Promise<void> {
    await this.db.run(`DELETE FROM ${MAJIKAH_SQL_TABLE_MAJIK_KEY}`);
  }

  async count(): Promise<number> {
    const row = await this.db.get<{ n: number }>(
      `SELECT COUNT(*) as n FROM ${MAJIKAH_SQL_TABLE_MAJIK_KEY}`,
    );
  ...
  async exists(id: string): Promise<boolean> {
    const row = await this.db.get(
      `SELECT 1 FROM ${MAJIKAH_SQL_TABLE_MAJIK_KEY} WHERE id = ?`,
      [id],
    );
  ...
  async query(query: StorageQuery<MajikKeyJSON>): Promise<MajikKeyJSON[]> {
    ...
    let sql = `SELECT json FROM ${MAJIKAH_SQL_TABLE_MAJIK_KEY}`;
